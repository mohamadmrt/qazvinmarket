            ...
            _search = function (page) {
                ...
                $.ajax({
                    ...
                    if(response.status==='ok'){
                        let orders=response.wallet_list;
                        let walletTotal=response.walletTotal;
                        let paginate=response.paginate;
                        let data='';
                        $.each(orders,function (key,item) {
                            data+=`<tr title="${item.status.label}" style="${item.status.id===4?'':'background-color:#ffdedb'}"><td>${item.id}</td>`;
