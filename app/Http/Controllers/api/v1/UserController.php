...
//            'confirm_code' => 'required|digits:4|exists:users,confirm',
    ...
    public function convertUserPoint(Request $request)
    {
        $user = auth()->user();
        ...
        switch ($request->type) {
            case(1):
                if ($request->point > $user->point) {
                    ...
                    foreach ($customer_club['levels'] as $key => $value) {
                        if ($value['min_score'] <= $user->point && $user->point < $value['max_score']) {

                            $credit = ($customer_club['convert_score_to_wallet_money'][$key]['amount'] * $request->point) / $customer_club['convert_score_to_wallet_money'][$key]['score'];
                            $user->update([
                            ...
                            return response()->json([
                                'data' => [],
                                'message' => 'کیف پول شما مبلغ ' . $credit . ' تومان شارژ شد',
                                'status' => 'ok'
                            ], 200);
                        }
                    }
